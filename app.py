...
irregular_verbs = {
    ...
    "do": {"past": "did", "3sg": "does", "ing": "doing"},
    "go": {"past": "went", "3sg": "goes", "ing": "going"},
    "say": {"past": "said", "3sg": "says", "ing": "saying"},
    "make": {"past": "made", "3sg": "makes", "ing": "making"},
    "know": {"past": "knew", "3sg": "knows", "ing": "knowing"},
    "think": {"past": "thought", "3sg": "thinks", "ing": "thinking"},
    "take": {"past": "took", "3sg": "takes", "ing": "taking"},
    "see": {"past": "saw", "3sg": "sees", "ing": "seeing"},
    "come": {"past": "came", "3sg": "comes", "ing": "coming"},
    "get": {"past": "got", "3sg": "gets", "ing": "getting"},
    "give": {"past": "gave", "3sg": "gives", "ing": "giving"},
    "find": {"past": "found", "3sg": "finds", "ing": "finding"},
    "tell": {"past": "told", "3sg": "tells", "ing": "telling"},
    "become": {"past": "became", "3sg": "becomes", "ing": "becoming"},
    "leave": {"past": "left", "3sg": "leaves", "ing": "leaving"},
    "feel": {"past": "felt", "3sg": "feels", "ing": "feeling"},
    "put": {"past": "put", "3sg": "puts", "ing": "putting"},
    "bring": {"past": "brought", "3sg": "brings", "ing": "bringing"},
    "begin": {"past": "began", "3sg": "begins", "ing": "beginning"},
    "keep": {"past": "kept", "3sg": "keeps", "ing": "keeping"},
    "hold": {"past": "held", "3sg": "holds", "ing": "holding"},
    "write": {"past": "wrote", "3sg": "writes", "ing": "writing"},
    "stand": {"past": "stood", "3sg": "stands", "ing": "standing"},
    "hear": {"past": "heard", "3sg": "hears", "ing": "hearing"},
    "let": {"past": "let", "3sg": "lets", "ing": "letting"},
    "mean": {"past": "meant", "3sg": "means", "ing": "meaning"},
    "set": {"past": "set", "3sg": "sets", "ing": "setting"},
    "run": {"past": "ran", "3sg": "runs", "ing": "running"},
    "cut": {"past": "cut", "3sg": "cuts", "ing": "cutting"},
    "read": {"past": "read", "3sg": "reads", "ing": "reading"},
    "swim": {"past": "swam", "3sg": "swims", "ing": "swimming"},
    "eat": {"past": "ate", "3sg": "eats", "ing": "eating"},
    "drink": {"past": "drank", "3sg": "drinks", "ing": "drinking"},
    "sleep": {"past": "slept", "3sg": "sleeps", "ing": "sleeping"},
    "sit": {"past": "sat", "3sg": "sits", "ing": "sitting"},
    "speak": {"past": "spoke", "3sg": "speaks", "ing": "speaking"},
    "drive": {"past": "drove", "3sg": "drives", "ing": "driving"},
    "choose": {"past": "chose", "3sg": "chooses", "ing": "choosing"},
}
...
### カスタムフィルタ
"""
@app.template_filter("highlight_word")
def highlight_word(text, word):
    ...
    # 単語の前後が単語境界（空白・記号など）のときのみマッチ（大文字小文字区別なし）
    pattern = re.compile(rf"\b({re.escape(word)})\b", flags=re.IGNORECASE)

    def replacer(match):
        return f'<span style="color:#02A08F; font-weight:bold;">{match.group(0)}</span>'

    pattern = re.compile(rf"\b({re.escape(word)})\b", flags=re.IGNORECASE)
    text = pattern.sub(replacer, text)

    return Markup(pattern.sub(replacer, text))
"""


@app.template_filter("highlight_word")
def highlight_word(text, base_form):
    if not text or not base_form:
        return text
    ...
    # 活用形の候補を生成
    forms_to_match = set()
    forms_to_match.add(base_form)

    # 不規則変化
    if base_form in irregular_verbs:
        forms_to_match.update(irregular_verbs[base_form].values())

    # 規則変化（s, ed, ing など）
    forms_to_match.add(base_form + "s")
    forms_to_match.add(base_form + "es")
    if base_form.endswith("y"):
        forms_to_match.add(base_form[:-1] + "ies")
        forms_to_match.add(base_form[:-1] + "ied")
    else:
        forms_to_match.add(base_form + "ed")
    forms_to_match.add(base_form + "ing")

    # 小文字化したセットでマッチ
    forms_to_match = set(w.lower() for w in forms_to_match)

    def replacer(match):
        return f'<span style="color:#02A08F; font-weight:bold;">{match.group(0)}</span>'

    for form in forms_to_match:
...
def submit():
    ...
    if user_answer == correct:
        # 正解の場合はカウンターに1を足す
        if current_user.is_authenticated:
            current_user.correct_answers = (current_user.correct_answers or 0) + 1
            db.session.commit()
